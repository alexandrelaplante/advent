data = """F--FJ77F-L7---J-F777-F7|77F|-J-F|77|F-L-J-77-L7FL77-7|----.F7-|-|.7-.-FL-FF-FF77.|.F-F-.F|.7.F.7-F-77.J7|----7LL7F|7|-F-J-777-77.F-7-FL77FF|
...
class Map:
    def __init__(self, data: str):
        rows = data.split("\n")
        big_map = [["=" for x in range(3 * len(rows[0]))] for y in range(3 * len(rows))]
        for y, row in enumerate(rows):
            for x, c in enumerate(row):
                submap = EMBIGGEN[c]
                big_map[y * 3][x * 3 : x * 3 + 3] = submap[0]
                big_map[y * 3 + 1][x * 3 : x * 3 + 3] = submap[1]
                big_map[y * 3 + 2][x * 3 : x * 3 + 3] = submap[2]
        self.rows = big_map
...
Step = namedtuple("Step", ["x", "y", "depth"])


def bfs(map: Map, start_x: int, start_y: int):
    to_visit = [Step(start_x, start_y, 0)]
    distances = [[None for x in range(map.width)] for y in range(map.height)]
    while len(to_visit) > 0:
        x, y, depth = to_visit.pop(0)
        distances[y][x] = depth
        for next_x, next_y in map.neighbours(x, y):
            if distances[next_y][next_x] is None:
                step = Step(next_x, next_y, depth + 1)
                if step not in to_visit:
...
def part1(data) -> int:
    ...
    distances = bfs(map, *map.find("S"))

    return max(max(n or 0 for n in row) for row in distances) // 3


def merge_in_place(distances1, distances2) -> None:
    for y in range(len(distances1)):
        for x in range(len(distances1[0])):
            has_num = distances1[y][x] is not None or distances2[y][x] is not None
            distances1[y][x] = 1 if has_num else None
...
def part2(data) -> int:
    ...
    distances = bfs(map, *map.find("S"))
    merge_in_place(distances, distances)

    # Transform all non-loop tiles into floor
    for y in range(map.height):
        for x in range(map.width):
            map.rows[y][x] = "+" if distances[y][x] is not None else "."

    # Flood-fill from the edges
    for y in range(map.height):
        for x in range(map.width):
            if x == 0 or y == 0:
                if distances[y][x] is None:
                    new_distances = bfs(map, x, y)
                    merge_in_place(distances, new_distances)

    # Anything left is enclosed by the loop
    # output = [[None for x in range(map.width // 3)] for y in range(map.height // 3)]
    total = 0
    for y in range(map.height // 3):
        for x in range(map.width // 3):
            if distances[y * 3 + 1][x * 3 + 1] is None:
                # output[y][x] = 1
                total += 1

    # for row in output:
    #     print("".join(str(s) if s is not None else "." for s in row))
