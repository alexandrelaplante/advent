...
class Part:
    ...
    def dict(self):
        return {
            "x": Range(start=self.x.start, end=self.x.end),
            "m": Range(start=self.m.start, end=self.m.end),
            "a": Range(start=self.a.start, end=self.a.end),
            "s": Range(start=self.s.start, end=self.s.end),
        }
...
class Rule:
    ...
    def matches(self, part: Part) -> tuple[Optional[Part], Optional[Part]]:
        """
        Part[0]: Parts that match
        Part[1]: Parts that don't match
        """
        if self.condition is None:
            return part, None
        elif ">" in self.condition:
            attr, ref = self.condition.split(">")
            ref = int(ref)
            ...
            # ..........#######0000000..........
            # ................>................
            matching_r = Range(start=ref + 1, end=val.end)
            unmatching_r = Range(start=val.start, end=ref)

            matching_part = Part(**{**part.dict, attr: matching_r})
            unmatching_part = Part(**{**part.dict, attr: unmatching_r})
            return matching_part, unmatching_part
            ...
            # ..........000000#######..........
            # ................<................
            matching_r = Range(start=val.start, end=ref - 1)
            unmatching_r = Range(start=ref, end=val.end)
...
def apply(workflows: list[Workflow], workflow_name: str, part: Part, rule_num: int = 0):
    workflow = next(w for w in workflows if w.name == workflow_name)
    rule = workflow.rules[rule_num]

    matching_part, unmatching_part = rule.matches(part)

    matching_total, unmatching_total = 0, 0

    if matching_part is not None:
        if rule.destination == "A":
            matching_total = matching_part.total
        elif rule.destination == "R":
            matching_total = 0
        else:
            matching_total = apply(
                workflows, workflow_name=rule.destination, part=matching_part
            )

    if unmatching_part is not None:
        unmatching_total = apply(
            workflows,
            workflow_name=workflow_name,
            rule_num=rule_num + 1,
            part=unmatching_part,
        )

    return matching_total + unmatching_total


def part2(data):
    workflows_str, _ = data.split("\n\n")
    workflows = []
    for workflow_str in workflows_str.split("\n"):
        ...
        for rule_str in rules_str[:-1].split(","):
            if ":" in rule_str:
                ...
                condition, destination = None, rule_str
            rules.append(Rule(condition=condition, destination=destination))
        workflows.append(Workflow(name=name, rules=rules))

    part = Part(x=Range(1, 4000), m=Range(1, 4000), a=Range(1, 4000), s=Range(1, 4000))

    total = apply(workflows=workflows, workflow_name="in", part=part)
