data = """Game 1: 2 red, 2 green; 1 red, 1 green, 2 blue; 3 blue, 3 red, 3 green; 1 blue, 3 green, 7 red; 5 red, 3 green, 1 blue
...
Game 100: 1 red, 5 blue, 2 green; 3 red, 1 blue; 1 green, 1 blue, 1 red""".split(
    "\n"
)
...
def part1():
    ...
    for row in data:
        id, rest = row[4:].split(":")
        reveals = rest.replace(";", ",").split(",")
        skip = False
        for reveal in reveals:
            num, color = reveal.strip().split(" ")
            if int(num) > COMPARISON[color]:
                skip = True
                break
        if not skip:
            total += int(id)
...
def part2():
    ...
    for row in data:
        id, rest = row[4:].split(":")
        reveals = rest.replace(";", ",").split(",")
        min_values = defaultdict(lambda: 0)
        for reveal in reveals:
            num, color = reveal.strip().split(" ")
            min_values[color] = max(int(num), min_values[color])
