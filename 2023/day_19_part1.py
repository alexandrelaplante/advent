...
def apply(workflows: list[Workflow], workflow_name: str, part: Part):
    ...
    for rule in workflow.rules:
        if rule.matches(part):
            ...
            if rule.destination == "R":
                return False
            return apply(workflows, workflow_name=rule.destination, part=part)
...
def part1(data):
    ...
    for part_str in parts_str.split("\n"):
        x_str, m_str, a_str, s_str = part_str[1:-1].split(",")
        x = int(x_str[2:])
        m = int(m_str[2:])
        a = int(a_str[2:])
        s = int(s_str[2:])
    ...
    total = 0
    for part in parts:
        if apply(workflows=workflows, workflow_name="in", part=part):
            total += part.sum
