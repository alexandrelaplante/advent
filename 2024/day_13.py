...
def solve_ilp_1(x1: int, x2: int, y1: int, y2: int, x_prize: int, y_prize: int) -> int:
    import pulp

    ilp = pulp.LpProblem("Problem", pulp.LpMinimize)
    ...
    # Constraints:
    ilp += x1 * A + x2 * B == x_prize
    ilp += y1 * A + y2 * B == y_prize
    ilp += A <= 100
    ilp += B <= 100

    status = ilp.solve(pulp.PULP_CBC_CMD(msg=False))
    if status == 1:
        tokens = int(pulp.value(ilp.objective))
        return tokens
    return 0
...
def part1(INPUT):
    ...
    for machine in INPUT.split("\n\n"):
        rules = machine.split("\n")
        x1 = re.search(r"X\+(\d*),", rules[0]).group(1)
        y1 = re.search(r"Y\+(\d*)$", rules[0]).group(1)

        x2 = re.search(r"X\+(\d*),", rules[1]).group(1)
        y2 = re.search(r"Y\+(\d*)$", rules[1]).group(1)

        prize_x = re.search(r"X=(\d*),", rules[2]).group(1)
        prize_y = re.search(r"Y=(\d*)$", rules[2]).group(1)
        total += solve_ilp_1(
            int(x1), int(x2), int(y1), int(y2), int(prize_x), int(prize_y)
        )

    print(total)


def solve_ilp_2(x1: int, x2: int, y1: int, y2: int, x_prize: int, y_prize: int) -> int:
    # pulp could not handle integers with values > 32 bits.
    from pyscipopt import Model

    # Create a model
    model = Model("Large Integer ILP")
    model.hideOutput()

    A = model.addVar(vtype="I", name="A")
    B = model.addVar(vtype="I", name="B")

    model.addCons(x1 * A + x2 * B == x_prize)
    model.addCons(y1 * A + y2 * B == y_prize)

    model.setObjective(3 * A + B, "minimize")

    model.optimize()

    # pyscipopt would return "optimal" results that that not satisfy the constraints
    # so I double check the constraints manually.
    if (
        model.getStatus() == "optimal"
        and (x1 * model.getVal(A) + x2 * model.getVal(B) == x_prize)
        and (y1 * model.getVal(A) + y2 * model.getVal(B) == y_prize)
    ):
        return int(model.getObjVal())
...
def part2(INPUT):
    ...
    for machine in INPUT.split("\n\n"):
        ...
        x_prize = re.search(r"X=(\d*),", rules[2]).group(1)
        y_prize = re.search(r"Y=(\d*)$", rules[2]).group(1)
        ...
        total += solve_ilp_2(
            ...
            int(y1),
            int(y2),
            int(x_prize) + 10000000000000,
            int(y_prize) + 10000000000000,
        )
