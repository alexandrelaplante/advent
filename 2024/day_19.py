...
def part1(INPUT):
    ...
    total = sum(is_possible(p) for p in patterns)
    print(total)
...
def part2(INPUT):
    ...
    def possibilities(pattern: str) -> int:
        ...
        for towel in towels:
            if pattern.startswith(towel):
                total += possibilities(pattern[len(towel) :])
        return total

    total = sum(possibilities(p) for p in patterns)
    print(total)
